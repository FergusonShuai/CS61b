...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    // Each cell of the maze is represented as a seperate item using DisjointSets
    DisjointSets cellSets = new DisjointSets(horiz * vert);
    

    // Use a array walls[] to reperesent all inner walls of a maze, and cells[] to represent all the cells.
    // The first (horiz - 1) * vert elements will store vertical walls, while
    // the rest (vert - 1) * horiz store horizontal walls.
    // The value of a wall is sum of the values of its two adjacent cells.
    int arraySize = (horiz - 1) * vert + (vert - 1) * horiz;
    /*
    int[] cells = new int[horiz *vert];
    for (j = 0; j < vert; j++) {
      for (i = 0; i < horiz; i++) {
        cells[j * horiz + i] = j * horiz + i;
      }
    }
    */
    int[] walls = new int[(horiz - 1) * vert + (vert - 1) * horiz];

    // Vertical walls
    for (j = 0; j < vert; j++) {
      for (i = 0; i < horiz - 1; i++) {
        walls[j*(horiz - 1) + i] = j * (horiz-1) + i;
      }
    }
    ...
    for (j = 0; j < vert - 1; j++) {
      for (i = 0; i < horiz; i++) {
        walls[(horiz - 1) * vert + j* (horiz) + i] = (horiz - 1) * vert + j * (horiz) + i;
      }
    }
    
    // Randomly choose an element from the walls and toss it to the end.
    int wallCounts = (horiz - 1) * vert + (vert - 1) * horiz;
    for (int w = wallCounts - 1; w > 0; w--) {
      
      int randomIndex = randInt(w+1);
      int v = walls[randomIndex];
      
      // Shifting all elements from index r to w one step forward to make room to the end of the array.
      for (int r = randomIndex; r < w; r++) {
        walls[r] = walls[r + 1];
      }

      // Placing the random-picked value at the end of the array
      walls[w] = v;
      // System.out.format("walls[%d]: %d\n", w, walls[w]);
    }

    for (int x = 0; x < (horiz - 1) * vert + (vert - 1) * horiz; x++ ) {
      System.out.format("walls[%d] = %d\n", x, walls[x]);
      if (walls[x] < (horiz - 1) * vert) {  // walls[x] is a vertical wall
        int xCoordinate = -1;
        int yCoordinate = -1;
        outerloop:
        for (j = 0; j < vert; j++) {
          for (i = 0; i < horiz - 1; i++) {
            if (j * (horiz - 1) + i == walls[x]) {
              xCoordinate = i;
              yCoordinate = j;
              break outerloop;
            } else {
        ...
        // Two adjacent cells' values
        int leftCell = horiz * yCoordinate + xCoordinate;
        int rightCell = horiz * yCoordinate + xCoordinate + 1;
        int rootLeft = cellSets.find(leftCell);
        int rootRight = cellSets.find(rightCell);
        if (rootLeft != rootRight) {  //There is no path connecting leftCell and rightCell
          cellSets.union(rootLeft, rootRight);
          vWalls[xCoordinate][yCoordinate] = false;
        }
      
      } else {  // walls[x] is a horizontal wall
        int xCoordinate = -1;
        int yCoordinate = -1;
        outerloop:
        for (j = 0; j < vert - 1; j++) {
          for (i = 0; i < horiz; i++) {
            if (j * horiz + i == walls[x] - (horiz - 1) * vert ) {
              xCoordinate = i;
              yCoordinate = j;
              break outerloop;
            } else {
        ...
        // Two adjacent cells' values
        int topCell = horiz * yCoordinate + xCoordinate;
        int bottomCell = horiz * (yCoordinate + 1) + xCoordinate;
        int rootTop = cellSets.find(topCell);
        int rootBottom = cellSets.find(bottomCell);
        if (rootTop != rootBottom) {  //There is no path connecting leftCell and rightCell
          cellSets.union(rootTop, rootBottom);
          hWalls[xCoordinate][yCoordinate] = false;
        }
      }
    }
    ...
    // Represent each cell of the maze as a seperate item using DisjointSets
