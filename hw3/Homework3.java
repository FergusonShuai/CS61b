...
public class Homework3 {
  ...
  public static void smoosh(int[] ints) {
    ...
    // int[] smooshedInts = new int[ints.length]; // This method doesn't return anything, so just need to modify ints.

    int i = 1;    // Pointer i used to keep track of updated elements in ints array;
    int oldNum = ints[i-1];
    for (int index = i; index < ints.length; index++){   // Pointer index is used to loop through original array ints.
      if (oldNum != ints[index]){
        ints[i] = ints[index];
        oldNum = ints[index];
        i++;
      }
